...
`<aside>`标签用来放置与网页或文章主要内容间接相关的部分。网页级别的`<aside>`，可以用来放置侧边栏；文章级别的`<aside>`，可以用来放置评论或注释。
...
一般来说，`<section>`都应该有标题，即包含`<h1>`~`<h6>`标签。另外，如果多个`<section>`联合在一起是有意义的，那么它们应该放置在同一个`<article>`里面。另一方面，一个`<section>`里面包含多个`<article>`，也是可以的。
